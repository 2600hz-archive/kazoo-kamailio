...
#define BLF_JSON_EVENT_CAT2 "Extra-Event-Category"
...
//static char mwi_body_buffer[256];
...
dbk_pres_user_t* dbk_pres_search_pu(unsigned int hash_code, str* user, str* event,
...
dbk_pres_user_t* dbk_new_pres_user(str* user, str* event, str* callid, str* localtag, str* remotetag, str* body)
{
	dbk_pres_user_t* pu = (dbk_pres_user_t*)shm_malloc(sizeof(dbk_pres_user_t) + user->len + event->len);
	...
	char* p = (char*)pu + sizeof(dbk_pres_user_t);
	pu->user.s = p;
...
int dbk_pres_insert_pu(str* user, str* event, str* callid, str* localtag, str* remotetag, str* body,
	unsigned int hash_code)
{
	dbk_pres_user_t* pu = dbk_new_pres_user(user, event, callid, localtag, remotetag, body);
...
int dbk_phtable_dialoginfo(str* local_user, str* local_event)
{
        dbk_pres_user_t* pu, *pu_prev = NULL;
	str* body = NULL;
        unsigned int hash_code = core_hash(local_user, local_event, dbk_phtable_size);
	str dlg_bodies[BLF_MAX_DIALOGS], event = str_init(local_event->s);
	int dlg_no = 0;

        lock_get(&dbk_phtable[hash_code].lock);
	pu = dbk_pres_search_pu(hash_code, local_user, local_event, &pu_prev, 0);
        if (pu != NULL && pu->pd != NULL) {
...
str* dbk_phtable_update(str* local_user, str* local_event, str* remote_user, str* callid,
	 str* localtag, str* remotetag, str* state, int initiator)
{
	...
	if (pu == NULL) {
		...
		if (!terminated) {
			...
			if ( dbk_pres_insert_pu(local_user, local_event, callid, localtag, remotetag, body, hash_code) < 0) {
	...
	if (pd == NULL) {
		...
		if (!terminated) {
			...
			pu = dbk_pres_search_pu(hash_code, local_user, local_event, 0, pu);
			if (pu == NULL) { /* pu deleted in the meantime (another dialog ended) */
				lock_release(&dbk_phtable[hash_code].lock);
				shm_free(pd);
				dbk_pres_insert_pu(local_user, local_event, callid, localtag, remotetag, body, hash_code);
				goto ret_current_dialog;
			}
			pd->next = pu->pd;
			pu->pd = pd;
...
int dbk_pres_update_and_notify(str* local_user, str* local_event, str* remote_user, str* callid,
	str* localtag, str* remotetag, str* state, int initiator) {
...
//	str event = str_init("dialog");
	...
	/* update in htable */
	if ((body = dbk_phtable_update(local_user, local_event, remote_user, callid, localtag,
	...
	/* call refresh_watchers from presence */
        if (presence_api.notify_watchers(local_user, local_event, body) < 0) {
		LM_ERR("Failed to notify watchers\n");
		goto error;
	}
	dbk_free_xml(body);
...
str* dbk_phtable_update_mwi(str* local_user, str* local_event, str* remote_user, str* callid,
...
int dbk_pres_update_and_notify_mwi(str* local_user, str* local_event, str* remote_user, str* callid,
	str* localtag, str* remotetag, str* mwi_account, str* mwi_waiting, str* mwi_new, str* mwi_saved, str* mwi_urgent, str* mwi_urgent_saved) {
...
//	str event = str_init("message-summary");
	...
	/* update in htable */
	if ((body = dbk_phtable_update_mwi(local_user, local_event, remote_user, callid, localtag,
...
int dbk_phtable_flush(int flush_all, str* user, str* event)
{
	dbk_pres_user_t* pu;
	...
	if (flush_all) {
		...
		hash_code = core_hash(user, event, dbk_phtable_size);
		lock_get(&dbk_phtable[hash_code].lock);
		pu = dbk_pres_search_pu(hash_code, user, event, &pu_prev, 0);
		if (pu == NULL) {
		...
		dbk_phtable_dialoginfo(user, event);
	}
	return 0;
}
...
int rmqp_pres_flush_handle(struct json_object* json_obj)
{
	...
	json_extract_field(BLF_JSON_EVENT_PKG, event);
	...
	return dbk_phtable_flush(flush_all, &user, &event);
}
...
struct mi_root * mi_dbk_phtable_flush(struct mi_root *cmd, void *param)
{
	...
	if (type.len == 3 && strncmp(type.s, "all", 3) == 0) {
		...
		if(event.s == NULL || event.len== 0) {
			LM_ERR( "No event tyoe provided\n");
			return init_mi_tree(404, "No event type provided", 20);
		}
		LM_DBG("Flush user [%.*s]\n", user.len, user.s);
	}

	if (dbk_phtable_flush(flush_all, &user, &event) < 0 ) {
...
void printjson(struct json_object *json_obj)
{
	json_object_object_foreach(json_obj, key, val)
	{
		LM_DBG("\t%s: %s\n", key, json_object_to_json_string(val));
	}
	LM_DBG("json.to_string()=%s\n", json_object_to_json_string(json_obj));
}
...
int rmqp_pres_update_handle(char* req)
{
	...
	} else if(event.len == str_event_message_summary.len && strncmp(event.s, str_event_message_summary.s, str_event_message_summary.len) == 0 ) {
              LM_DBG("message-summary from kazoo");
...
//     ret = dbk_pres_update_and_notify_mwi(&from_user, &to_user, &callid, &fromtag, &totag, &mwi_account, &mwi_waiting, &mwi_new, &mwi_saved, &mwi_urgent, &mwi_urgent_saved);
       str* mwibody = dbk_phtable_update_mwi(&from_user, &event_name, &to_user, &callid, &fromtag, &totag, &mwi_account, &mwi_waiting, &mwi_new, &mwi_saved, &mwi_urgent, &mwi_urgent_saved);
//     str* mwibody = mwi_body(&mwi_account, &mwi_waiting, &mwi_new, &mwi_saved, &mwi_urgent, &mwi_urgent_saved );
       ...
       free_mwi_body(mwibody);
        ...
        str local_user, remote_user, local_tag, remote_tag;
        int initiator;
...
//      str* body = pua_dialoginfo_api.build_dialoginfo(state.s, &local_user, &remote_user,	&callid, initiator, &local_tag, &remote_tag, 0 , 0, 0);
        str* body = dbk_phtable_update(&local_user, &event_name, &remote_user, &callid, &local_tag, &remote_tag, &state, initiator);
		if ( body == NULL) {
				LM_ERR("Failed to construct BLF XML doc\n");
				goto error;
			}
        ...
        publ1.pres_uri= &local_user;
       	publ1.body= body;
	    publ1.event= DIALOG_EVENT;
        publ1.expires = 3600;
	    pua_api.send_publish(&publ1);
        dbk_free_xml(body);
		...
		const db_val_t* _v, const db_key_t* _c, int _n, int _nc, db1_res_t** _r)
{
	...
	LM_DBG("dbk_presence_query %s %s %s\n", username.s, domain.s, pres_uri.s);
	...
	hash_code = core_hash(&pres_uri, &event, dbk_phtable_size);

	lock_get(&dbk_phtable[hash_code].lock);
	pu = dbk_pres_search_pu(hash_code, &pres_uri, &event, 0, 0);
	...
	LM_DBG("The user has %d active dialogs\n", row_cnt);
				...
				const db_val_t* db_val, const int _n)
{
	...
	LM_DBG("from_user=[%.*s] to_user=[%.*s] callid=[%.*s] state=[%.*s]\n",
		from_user.len, from_user.s, to_user.len, to_user.s, callid.len, callid.s, state.len, state.s);
...
int dbk_presence_subscribe_alert_kazoo2(rmq_conn_t* rconn, str* user,unsigned int expires, str* from_user, str* event, str* contact, str* callid, str* from_tag, str* to_tag)
{
	...
	str dlg_event = str_init("dialog");
	str dlg_presence = str_init("presence");

	str local_event = str_init("presence");

	if (event->len == dlg_event.len && strncmp(dlg_event.s, event->s, dlg_event.len) == 0) {
			local_event = dlg_presence;
       } else {
         local_event = *event;
       }
...
//	json_object_object_add(json_object, BLF_JSON_EVENT_CATEGORY, json_object_new_string_len(event->s, event->len));
    json_object_object_add(json_object, BLF_JSON_EVENT_CATEGORY, json_object_new_string("presence"));
	json_object_object_add(json_object, BLF_JSON_EVENT_NAME, json_object_new_string("subscription"));
	json_object_object_add(json_object, BLF_JSON_EVENT_PKG, json_object_new_string_len(local_event.s, local_event.len));
	tmb.generate_callid(&unique_string);
	json_object_object_add(json_object, BLF_JSON_MSG_ID, json_object_new_string_len(unique_string.s, unique_string.len));
...
{
       str event = STR_STATIC_INIT("message-summary");
       str tag = STR_STATIC_INIT("");

       return dbk_presence_subscribe_alert_kazoo2(rconn, user, expires, from_user, &event, NULL,NULL, &tag, &tag);
...
//	}
	...
	return dbk_presence_subscribe_alert_kazoo2((rmq_conn_t*)_h->tail, &user,
				...
				const int _n, const int _un)
{
	...
	for (i = 0; i< _un; i++) {
		if (_uk[i]->len == str_expires_col.len && strncmp(_uk[i]->s, str_expires_col.s, str_expires_col.len) == 0) {
			...
			to_tag = _uv[i].val.str_val;
              ...
              LM_DBG("subscribe field %s", _uk[i]->s);
...
*/
	...
	t = tmb.t_gett();
	if (t == NULL) {
		LM_DBG("No tm transaction or no sip msg found\n");
		return -1;
	}
	if (t == NULL || t->uas.request == NULL) {
		LM_DBG("No tm transaction or no sip msg found\n");
	...
	if (pv_get_spec_value(t->uas.request, &sht_spec, &value) < 0) {
		LM_DBG("Failed to get sht value\n");
		return -1;
	}

	/* set it again to reset expires */
	if (pv_set_spec_value(t->uas.request, &sht_spec, 0, &value) < 0) {
		LM_DBG("Failed to add sht value\n");
	}
	if (value.rs.len == 0) {
