...
db1_res_t *dbk_creds_build_result(char *body, const db_key_t * _c, int _nc) {
	...
	LM_ERR("Error parsing json: %s\n", json_tokener_errors[-(unsigned long)
							       json_obj]);
	goto error;
    }

    init_json_obj = json_obj;
    struct json_object *password_obj = json_object_object_get(json_obj,
							      "Auth-Password");
    if (is_error(json_obj)) {
	LM_ERR("Error parsing json: %s\n", json_tokener_errors[-(unsigned long)
							       json_obj]);
	...
	LM_DBG
	    ("Allocated %lu bytes for RES_NAMES[%d] at %p\n",
	...
	if (strncmp(_c[col]->s, "password", _c[col]->len) == 0) {
		...
		(char *)pkg_malloc(password.len + 1);
...
int dbk_credentials_query(const db1_con_t * _h, const db_key_t * _k,
			  const db_val_t * _v, const db_key_t * _c,
                          int _n, int _nc, db1_res_t ** _r) {
    ...
    amqp_rk.len =
	amqp_util_encode(&_v[1].val.str_val,
			 routingkey + AUTH_ROUTING_KEY_PREFIX_LEN) - routingkey;
    ...
    sprintf(serverid,
	    "kamailio@%.*s-<%d>-%lu",
	    _v[1].val.str_val.len,
	    _v[1].val.str_val.s, my_pid(), rpl_routing_key_count++);
	...
	sprintf(messagebody,
		...
		_v[0].val.str_val.s,
		_v[1].val.str_val.len,
		_v[1].val.str_val.s, serverid,
		dbk_node_hostname.len,
		dbk_node_hostname.s,
		unique_string.len, unique_string.s, VERSION, NAME);
    ...
    amqp_bytes_t rpl_routing_key = {
	strlen(serverid), serverid
    };

    LM_DBG
	("AMQP: before queue_declare rpl_routing_key: [%.*s]\n",
	 (int)rpl_routing_key.len, (char *)rpl_routing_key.bytes);
    amqp_queue_declare_ok_t *r = amqp_queue_declare(rmq->conn,
						    rmq->channel,
						    amqp_empty_bytes, 0,
						    0, 1, 1,
						    amqp_empty_table);
    ...
    amqp_bytes_t rpl_exch = {
	8, "targeted"
    };

    if (amqp_queue_bind
	(rmq->conn, rmq->channel, rpl_queue,
	 rpl_exch, rpl_routing_key,
	 amqp_empty_table) < 0
    ...
    if (amqp_basic_consume
	(rmq->conn, rmq->channel, rpl_queue,
	 amqp_empty_bytes, 0, 1, 1,
	 amqp_empty_table) < 0
    ...
    LM_DBG
	("AMQP: after queue_bind  rpl_routing_key: [%.*s]\n",
	 (int)rpl_routing_key.len, (char *)rpl_routing_key.bytes);
	...
	(rmq->conn, rmq->channel,
	 rmq->exchange, amqp_rk, 0, 0,
	 &props, amqp_mb) < 0
	...
	if (body_received != body_target) {
	    LM_CRIT
		("Unexpected body size: recv %zu, expected %zu\n",
 ...
 */
int db_kazoo_query(const db1_con_t * _h, const db_key_t * _k,
...
int amqp_register_notice(const db1_con_t * _h, const db_key_t * _k,
	...
	if (!user.len && strncmp(_k[col]->s, "username", 8) == 0) {
	    ...
	    if (!user_agent.len && strncmp(_k[col]->s, "user_agent", 10) == 0) {
	    ...
	    if (!_v[col].nul) {
		char *dp = memchr(_v[col].val.str_val.s, ':',
				  _v[col].val.str_val.len);
		if (dp == NULL) {
		    LM_ERR
			("Wrong formated socket uri %.*s\n",
		...
		network_ip.s = dp;
		dp = memchr(dp, ':',
			    _v[col].val.str_val.len - network_proto.len - 1);
		if (dp == NULL) {
		    LM_ERR
			("Wrong formated socket uri %.*s\n",
			 _v[col].val.str_val.len, _v[col].val.str_val.s);
		...
		network_port.s = dp + 1;
		network_port.len =
		    _v[col].val.str_val.s +
		    _v[col].val.str_val.len - network_port.s;
	    } else {
	...
	sprintf(messagebody,
		...
		"\"Event-Category\":\"directory\"} ",
		network_port.len,
		network_port.s, network_ip.len,
		network_ip.s,
		dbk_node_hostname.len,
		dbk_node_hostname.s, host.len,
		host.s, user.len, user.s,
		host.len, host.s, user.len,
		user.s, callid.len, callid.s,
		user_agent.len, user_agent.s,
		network_proto.len,
		network_proto.s, host.len,
		host.s, user.len, user.s,
		expires, user.len, user.s,
		user.len, user.s,
		received.len - 4, received.s + 4, fs_path.len, fs_path.s,
    ...
    amqp_rk.len =
	amqp_util_encode(&host, routingkey + REG_ROUTING_KEY_PREFIX_LEN)
	- routingkey;
    routingkey[amqp_rk.len++] = '.';
    amqp_rk.len =
      amqp_util_encode(&user, routingkey + amqp_rk.len) - routingkey;
    ...
    if (amqp_basic_publish
	(rmq->conn, rmq->channel,
	 rmq->exchange, amqp_rk, 0, 0, &props, amqp_mb) < 0) {
